...
struct irq_keydev
{
    ...
    //struct tasklet_struct tasklet; //中断下半部结构体 tasklet_struct
    struct work_struct work; //中断下半部结构体 work_struct
};
...
static irqreturn_t key0_handler(int irq, void *parm){
    ...
    flag = schedule_work(&dev->irq_key[0].work);
    return IRQ_HANDLED;
}
...
static void workStruct_handler(struct work_struct *work){
    printk("work_hanlder\r\n");
    mod.mytimer.data = (volatile unsigned long)&mod;
    mod_timer(&mod.mytimer , jiffies + msecs_to_jiffies(10));
}
...
static int __init mod_init(void){
    ...
    for(i = 0; i < KEYNUM; i++){
        ...
        INIT_WORK(&mod.irq_key[i].work , workStruct_handler);
    }
