...
#include <linux/wait.h> //Linux阻塞访问
#include <linux/ide.h> //Linux阻塞访问
#include <linux/poll.h> //linux非阻塞访问

/*设备数量和设备名称定义*/
#define MOD_CNT 1
#define MOD_NAME "nonblockio"
...
static ssize_t mod_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos){
    ...
    if(filp->f_flags & O_NONBLOCK){ //非阻塞
        if(atomic_read(&dev->releasekey) == 0){ //按键还没释放 无效访问
            printk("abnormally access\r\n");
            return -EAGAIN;
        }
    }
    else{ //阻塞访问
...
#if 0
    //使用等待队列项来唤醒
    DECLARE_WAITQUEUE(wait,current); //定义wait的队列项目名字，并标记该等待队列项属于当前进程
        
    //进程读取文件一开始使其进入休眠太
    add_wait_queue(&dev->r_wait , &wait); //将该进程加入到休眠态
    __set_current_state(TASK_INTERRUPTIBLE); //设置当前进程的状态为可被打断状态，方便kill
    schedule();//切换进程

    //唤醒以后从这里运行
    if(signal_pending(current)){//非正常唤醒
        printk("abnormally wake up\r\n");
        ret = -EINTR;
        goto out;
    }
...
#if 0
out:
    __set_current_state(TASK_RUNNING); /*先将进程设置为运行态*/
...
static unsigned int mod_poll(struct file *filp, struct poll_table_struct *wait){
    int ret = 0;
    ...
    /*驱动是否可读*/
    if(atomic_read(&dev->releasekey)){ //按键按下 可读状态
        ret = POLLIN | POLLRDNORM;  /*通知进程可读*/
    }
...
static int __init mod_init(void){
    ...
    //初始化等待队列头
    init_waitqueue_head(&mod.r_wait);
